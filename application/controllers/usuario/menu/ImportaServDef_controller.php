...
class ImportaServDef_controller extends Sistema_Controller {
    ...
    {
        $data = array();
        $this->usuario_view('ImportaServDef_view',$data);
    }
    ...
    public function upload() 
    {
        ...
        $data['breadcrumbs'] = array('Home' => '#');
         ...
         $this->form_validation->set_rules('fileURL_serv_def', 'Upload File', 'callback_checkFileValidation_serv_def');
         ...
         {  
            // If file uploaded
            if(!empty($_FILES['fileURL_serv_def']['name'])): 

                // get file extension
                $extension = pathinfo($_FILES['fileURL_serv_def']['name'], PATHINFO_EXTENSION);
                ...
                // file path
                $spreadsheet = $reader->load($_FILES['fileURL_serv_def']['tmp_name']);
                $allDataInSheet = $spreadsheet->getActiveSheet()->toArray(null, true, true, true);
            
                // array Count
                $arrayCount = count($allDataInSheet);
                $flag = 0;
                $createArray = array('Filial', 'Data', 'Vendedor', 'Cliente', 'CPF', 'Servico', 'Plano', 'Acesso', 'Email', 'Next', 'GED', 'Valor', 'Remuneracao', 'Grupo');
                $makeArray = array('Filial' => 'Filial', 'Data' => 'Data', 'Vendedor' => 'Vendedor', 'Cliente' => 'Cliente', 'CPF' => 'CPF', 'Servico' => 'Servico', 'Plano' => 'Plano', 'Acesso' => 'Acesso', 'Email' => 'Email', 'Next' => 'Next', 'GED' => 'GED', 'Valor' => 'Valor', 'Remuneracao' => 'Remuneracao', 'Grupo' => 'Grupo');
                $SheetDataKey = array();
                ...
                if ($flag == 1) 
                {
                    for ($i = 2; $i <= $arrayCount; $i++)
                    {
                        ...
                        $fetchData[] = array('filial_vendas_servicos' => $filial, 'data_vendas_servicos' => data_para_banco($data), 'vendedor_vendas_servicos' => $vendedor, 'cliente_vendas_servicos' => $cliente, 'cpf_vendas_servicos' => $cpf, 'servico_vendas_servicos' => $servico, 'plano_vendas_servicos' => $plano, 'acesso_vendas_servicos' => $acesso, 'email_vendas_servicos' => $email, 'next_vendas_servicos' => $next, 'ged_vendas_servicos' => $ged, 'valor_vendas_servicos' => $valor, 'remuneracao_vendas_servicos' => $remuneracao, 'grupo_vendas_servicos' => $grupo);
                    }

                    $dados['dataInfo'] = $fetchData;
                    //$this->Analista_model->exclui_informacoes('servicos');
                    $this->Analista_model->setBatchImport($fetchData);
                    $this->Analista_model->importData(4);
                }
                else 
                {
                ...
                $this->usuario_view('ServDef_view', $dados);
    ...
    // checkFileValidation_serv_def
    public function checkFileValidation_serv_def($string)
    {
      ...
      if(isset($_FILES['fileURL_serv_def']['name']))
      {
            $arr_file = explode('.', $_FILES['fileURL_serv_def']['name']);
            $extension = end($arr_file);
            if(($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv') && in_array($_FILES['fileURL_serv_def']['type'], $file_mimes)){
                return true;
            }
            else
            {
                $this->form_validation->set_message('checkFileValidation_serv_def', 'Formato de arquivo incorreto! Apenas .xlsx, xls e csv');
                return false;
            }
        }
      else
        {
            $this->form_validation->set_message('checkFileValidation_serv_def', 'Por favor, selecione algum arquivo.');
            return false;
        }
    }
}
